...
@ThreadSafe
@Beta
public abstract class RateLimiter1 {
  /*
  ...
  public static RateLimiter1 create(double permitsPerSecond) {
    /*
  ...
  @VisibleForTesting
  static RateLimiter1 create(SleepingTicker ticker, double permitsPerSecond) {
    RateLimiter1 rateLimiter = new Bursty(ticker, 10 /* maxBurstSeconds */);
   ...
   */
  public static RateLimiter1 create(double permitsPerSecond, long warmupPeriod, TimeUnit unit) {
    return create(SleepingTicker.SYSTEM_TICKER, permitsPerSecond, warmupPeriod, unit);
  }

  @VisibleForTesting
  static RateLimiter1 create(
      SleepingTicker ticker, double permitsPerSecond, long warmupPeriod, TimeUnit unit) {
    RateLimiter1 rateLimiter = new WarmingUp(ticker, warmupPeriod, unit);
    rateLimiter.setRate(permitsPerSecond);
    return rateLimiter;
  }

  @VisibleForTesting
  static RateLimiter1 createWithCapacity(
  ...
  private RateLimiter1(SleepingTicker ticker) {
    this.ticker = ticker;
  ...
  /**
   * Acquires permits from this {@link RateLimiter1} if it can be acquired immediately without delay.
   *
   * <p>
  ...
  /**
   * Acquires a permit from this {@link RateLimiter1} if it can be acquired immediately without
   * delay.
   *
   * <p>
   ...
   */
  private static class WarmingUp extends RateLimiter1 {
   ...
   */
  private static class Bursty extends RateLimiter1 {
